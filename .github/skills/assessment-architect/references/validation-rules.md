...
```
FOR each question:
  option_lengths = [len(A), len(B), len(C), len(D)]
  correct_rank = rank of correct answer by length (0=shortest, 3=longest)
  store correct_rank

count_longest = count where correct_rank == 3
IF count_longest / total > 0.40: FAIL "Correct answer is longest option {count_longest}/{total} times (length bias)"

count_shortest = count where correct_rank == 0
