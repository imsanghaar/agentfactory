...
│
├── build-source/          # 🔨 BUILD ARTIFACTS (gitignored)
│   │                      # - Hydrated content from PanaversityFS
│   │                      # - Generated by CI workflow
│   │                      # - Docusaurus reads from here when PanaversityFS enabled
│   │
├── panaversity-fs/        # MCP server + build scripts
│   ├── app/               # FastAPI server
│   ├── scripts/
│   │   ├── ingest-book.py  # Uploads docs/ → PanaversityFS
│   │   └── hydrate-book.py # Downloads PanaversityFS → build-source/
│   └── tests/
│
└── .panaversity/          # 📋 MANIFEST CACHE (gitignored)
    └── manifest.json      # Tracks last build state for delta detection
```
...
Author pushes to apps/learn-app/docs/
    ...
    │  2. Run ingest-book.py              │
    │  3. Upload ONLY changed files       │
    └─────────────────────────────────────┘
         │
         ▼
    ┌─────────────────────────────────────┐
    │      PanaversityFS (R2 Storage)     │
    │  ─────────────────────────────────  │
    │  - book-id: ai-native-dev           │
    │  - Delta detection via hashes       │
    │  - MCP tools for read/write         │
    └─────────────────────────────────────┘
         │
         ▼
    ┌─────────────────────────────────────┐
    ...
    │  ─────────────────────────────────  │
    │  1. Restore manifest cache          │
    │  2. Run hydrate-book.py             │
    │  3. Download ONLY delta to          │
    │     build-source/                   │
    │  4. Fallback to docs/ if fails      │
    │  5. Docusaurus reads build-source/  │
    │  6. Build & deploy                  │
    └─────────────────────────────────────┘
```
...
```
book-source/
├── docs/       # Authors write here
└── src/        # UI developers work here
```
...
├── Part-01/
├── Part-02/
└── .github/workflows/sync.yml    # Syncs to PanaversityFS

Repo 2: book-interface/            # UI only
├── src/
├── docusaurus.config.ts
└── .github/workflows/deploy.yml   # Hydrates from PanaversityFS
```
...
✓ Sync: "Uploaded 3 files in 4.2s"
✓ Hydrate: "Downloaded 5 files (delta) in 8.1s"
✓ Build: "Build completed in 1m 34s"
```

### Cache Hit Rate

```
Manifest cache hit → Incremental download (fast)
Manifest cache miss → Full download (slower, but rare)
...
**Old behavior:** Docusaurus plugin fetches all 1000+ files → timeout
...
- **Need to force full sync?** Workflow dispatch → sync-content.yml → Enable "full_sync"
